...
def compute_HIC(t, a):
    """
    计算头部加速度 a(t) 对应的 HIC 值。
    这里 a 应该是以 m/s² 为单位，我们将其转换为 g 的单位 (1g ≈ 9.81 m/s²)。

    参数:
        t: 时间数组 (秒)
        a: 头部合成加速度数组 (m/s²)
    返回:
        hic_value: 计算得到的 HIC 值
    """
    # 转换加速度单位，从 m/s² 到 g
    a_g = a / 9.81
    ...
    for i in range(n - 1):
        for j in range(i + 1, n):
            ...
            a_avg = area_ij / dt_ij  # 平均加速度 (单位: g)
...
# 假设你之前已经定义了 solution() 和 compute_injury_metrics() 等函数

# -------------------- 多参数联合扫描函数 --------------------
def scan_multiple_parameters(param_names, param_ranges, fixed_params, init_conditions, t_max):
    """
    同时扫描多个参数的取值范围，并提取每个参数组合下的关键伤害指标。
    返回:
        combos: 每个参数组合（字典形式）的列表
        metrics_list: 每个组合对应的指标字典列表
    """
    combos = []
    metrics_list = []
...
def pareto_front_indices(objectives):
    ...
    for i in range(n_points):
        for j in range(n_points):
            if i != j:
                # 如果 j 在所有目标上都不劣于 i 且至少在一个目标上更优，则 i 被支配
...
# 固定其它参数（baseline 参数），参见你的 baseline 参数字典
baseline_params = {
